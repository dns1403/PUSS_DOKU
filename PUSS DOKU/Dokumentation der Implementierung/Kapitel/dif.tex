...
Da ein Unterricht (identifiziert über die Unterrichtsnummer) aus mehreren Zeilen bestehen kann (z.B. bei Klassenkopellungen oder Team-Teaching), ist die Unterrichtsnummer allein nicht eindeutig. Ein Datensatz wird durch die Kombination aus Unterrichtsnummer, Klasse und Lehrer eindeutig bestimmt.

Im Kontext des PUSS Tools wird diese Datei primär genutzt, um die \textbf{Studentenzahl} (Hörerzahl) für die Berechnung der Raumauslastung zu ermitteln.
...
\begin{table}[h]
    ...
    \begin{tabular}{|c|c|c|c|c|c|}
        ...
        \textbf{Constraints}& NN, PK & NN, PK & NN, PK & NN &  \\
...
\begin{itemize}
    ...
    \item \textbf{Studentenzahl:} Gibt an, wie viele Schüler an diesem spezifischen Unterrichtsteil teilnehmen. Dies ist essenziell für die Kapazitätsplanung der Räume im Zielsystem.
\end{itemize}
\subsection{GPU003 -- Klassen}
Die Datei \texttt{GPU003.TXT} enthält die Stammdaten der Klassen. Im Kontext des PUSS Tools werden diese Daten auf die Entität \texttt{Studiengaenge} abgebildet. Eine Klasse entspricht dabei einer Kohorte oder einem Studiengangs-Jahrgang.
...
\paragraph{Erläuterung der relevanten Felder:}
\begin{itemize}
    \item \textbf{Name:} Das Kürzel der Klasse (z.B. "05A"). Dient als Primärschlüssel und wird im Zielsystem zur \texttt{StudiengangsID}.
...
Die Datei \texttt{GPU004.TXT} beinhaltet die Lehrer-Stammdaten. Diese werden im Zielsystem als \texttt{Professor} interpretiert.

Der Identifikator ist das Lehrerkürzel (\texttt{Name}). Obwohl die Datei viele Attribute für die statistische Auswertung und Abrechnung enthält (z.B. Hohlstunden, Sollarbeitszeit), sind für die Raum- und Zeitplanung im PUSS Tool primär das Kürzel und der Klarname relevant.
...
\begin{itemize}
    ...
    \item \textbf{Langname, Vorname, Titel:} Dienen der Anzeige im Zielsystem (z.B. "Dr. Hans Müller"). Im aktuellen Mapping-Skript wird vereinfachend oft nur das Kürzel verwendet, die Stammdaten stehen hier jedoch für eine Erweiterung bereit.
\end{itemize}
\subsection{GPU005 -- Räume}
Die Datei \texttt{GPU005.TXT} enthält die Stammdaten der Räume. Im PUSS Tool werden diese Daten auf die Entität \texttt{Raum} abgebildet.
...
\paragraph{Erläuterung der relevanten Felder:}
\begin{itemize}
    \item \textbf{Name:} Die eindeutige Bezeichnung des Raumes (z.B. "R101"). Dient als Primärschlüssel und wird im Zielsystem zur \texttt{Raumnr}.
    \item \textbf{Kapazität:} Die maximale Anzahl an Personen (Sitzplätze). Dies wird auf \texttt{Sitzplatzzahl} gemappt.
    \item \textbf{Gang 1 / Gang 2:} Diese Felder enthalten oft Informationen über das Gebäude oder das Stockwerk. Im Mapping-Prozess werden diese beiden Felder kombiniert (konkateniert), um das Attribut \texttt{Gebaeude} im Zielsystem zu befüllen.
\end{itemize}
\subsection{GPU006 -- Fächer}
Die Datei \texttt{GPU006.TXT} enthält die Stammdaten der Unterrichtsfächer. Im PUSS Tool werden diese Daten auf die Entität \texttt{Kurs} abgebildet.
...
\paragraph{Erläuterung der relevanten Felder:}
\begin{itemize}
    \item \textbf{Name:} Das eindeutige Kürzel des Faches (z.B. "M" für Mathematik). Dient als Primärschlüssel und wird im Zielsystem zur \texttt{KursID}.
...
Im PUSS Tool wird diese Datei verwendet, um die Tabelle \texttt{Pflicht} zu generieren. Durch den Abgleich des Stundentafel-Namens mit der Schulform einer Klasse kann ermittelt werden, welche Kurse für einen Studiengang obligatorisch sind.
...
\begin{itemize}
    ...
    \item \textbf{Fach:} Das Kürzel des Unterrichtsfaches. Dies ist ein Fremdschlüssel auf die Datei \texttt{GPU006} (Fächer) und wird im Zielsystem zur \texttt{KursID}.
...
Im PUSS Tool ist diese Datei essenziell für die Berechnung der Spalte \texttt{Wunschzeit} in der Tabelle \texttt{Plan}. Ein Datensatz definiert dabei für eine bestimmte Entität und einen bestimmten Zeitschlitz (Tag + Stunde) einen Prioritätswert.
...
\begin{itemize}
    ...
    \item \textbf{Zeitwunsch:} Ein numerischer Wert, der die Priorität ausdrückt. Typische Untis-Skala:
        ...
        \end{itemize}
\end{itemize}
\subsection{Hilfsrelationen (Generiert)}
Die folgenden Relationen existieren nicht als physische DIF-Dateien. Sie werden während des Importprozesses generiert, um proprietäre Untis-Kodierungen (wie numerische Wochentage oder Unterrichtsstunden) in das Format der Zieldatenbank (STULP/PUSS) zu übersetzen.
...
Die Tabelle \texttt{WeekdayMap} dient der Übersetzung der numerischen Wochentags-Repräsentation aus Untis in lesbare Textformate für das Zielsystem.
...
Die Tabelle \texttt{ClassEncoding} ist eine spezifische Hilfstabelle für das PUSS Tool. Sie ordnet den Untis-Klassennamen (z.B. "05A") einer logischen Veranstaltungsgruppe (Veranstaltungsteil) zu. Dies ermöglicht es, Parallelklassen (05A, 05B etc.) im Zielsystem sauber zu trennen oder Oberstufenkurse (Q1, Q2) spezifisch zu mappen.
...
\begin{itemize}
    ...
    \item \textbf{Veranstaltungsteil:} Ein interner Identifikator für das Zielsystem, der hilft, Kurse eindeutig Studiengangs-Instanzen zuzuordnen (z.B. wird die Klasse 05A zum Veranstaltungsteil 5.1).
\end{itemize}
