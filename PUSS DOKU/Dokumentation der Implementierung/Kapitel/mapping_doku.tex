...
\label{sec:mapping}
In diesem Kapitel wollen wir zwei Python-Skripte vorstellen, die den Kern des Datenmappings bilden. 
Das erste Skript \todo{Namen entgültig festlegen} bildet die relevanten Daten aus den Untis-DIF-Textdateien in eine relationale SQLite-Datenbank (\texttt{Untis.db}) um. 
...
Das Mapping-Skript \texttt{dif\_to\_stulp.py} realisiert den ETL-Prozess (Extract, Transform, Load), um die Rohdaten aus der Untis-Export-Datenbank (\texttt{Untis-DIF.db}) in das Zielschema des (lokalen) Planungstools (\texttt{STULP.db}) zu überführen.
Der Prozess verbindet sich mit beiden SQLite-Datenbanken und führt SQL-basierte Transformationen durch.
\subsection{Datenextraktion und -bereinigung aus Untis-DIF}
...
\todo{Skript wenn fertig fertig hier einfügen}

\subsection{Mapping der Daten in das Planungstool}
...
\subsubsection{Schema Professor}
\begin{minted}[
    ...
    label={SQL-Logik für Professor} % Die Überschrift des Kastens
]{sql}
