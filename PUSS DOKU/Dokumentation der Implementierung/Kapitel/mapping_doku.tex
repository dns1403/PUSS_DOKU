...
Das Mapping-Skript \texttt{dif\_to\_stulp.py} dient genau diesem Zweck. Es führt SQL-basierte Transformationen durchzuführen, um die Daten in das Zielschema des Planungstools zu überführen.
...
Die Spalte \texttt{Schul-Stufe} wird als Semester interpretiert, da sie die Jahrgangsstufe beschreibt, was im Planungstool als Semester abgebildet wird.
% \begin{itemize}
...
\subsection{Zeitfenster (Timeslot)}
...
Durch den \texttt{JOIN} mit den Hilfstabellen \texttt{HourGrid} und \texttt{WeekdayMap} werden die internen Untis-Codes (z.B. Tag 1, Stunde 1) in lesbare Werte (Montag, 08:00 Uhr, 45 Min) übersetzt.
Die SQL-Fensterfunktion \texttt{ROW\_NUMBER()} erzeugt dabei einen fortlaufenden Primärschlüssel.
...
\subsection{Planungsdaten (Plan)}
Die Tabelle \texttt{Plan} repräsentiert den aktuellen Ist-Zustand des Stundenplans.
Die Transformation ist hierbei die anspruchsvollste des gesamten ETL-Prozesses, da die Datenstruktur von Untis (atomare Einzelstunden pro Klasse) fundamental von der des Planungstools (zusammenhängende Blöcke pro Veranstaltung) abweicht.
...
\begin{enumerate}
    \item \textbf{Behandlung von \_Kursen:} Klassenübergreifende Kurse (z.B. Religion, Wahlpflicht) sind in Untis oft durch ein führendes Unterstrich-Zeichen im Fachkürzel markiert (z.B. \texttt{\_REL}). Würde man hier das reguläre Mapping anwenden, entstünden für jede teilnehmende Klasse separate Einträge. Das Skript erkennt dies und erzwingt für solche Fächer den \texttt{Veranstaltungsteil} '1'.
...
Die äußere Abfrage gruppiert die vorverarbeiteten Datensätze. Während Untis eine Doppelstunde als zwei separate Zeilen speichert (z.B. 1. Stunde und 2. Stunde), benötigt das Planungstool einen einzigen Eintrag mit addierter Dauer.
...
\subsection{Pflichtbindungen (Pflicht)}
Die Tabelle \texttt{Pflicht} verknüpft Kurse mit Studiengängen (Klassen) und definiert, ob es sich um Pflichtveranstaltungen handelt. Diese Information ist essenziell für die Konflikterkennung (z.B. "StudiengangSchwer").
...
Über die Schulform der Klasse (z.B. "G9" aus \texttt{dif\_data.Klassen}) wird die Verbindung zwischen der Klasse (StudiengangsID) und der Stundentafel hergestellt.

\subsubsection{Quelle 2: Faktischer Unterricht}
Da Schulplanung oft dynamisch ist, existieren häufig Kurse im Stundenplan, die nicht explizit in der Stundentafel stehen (z.B. AGs, Förderunterricht oder kurzfristige Änderungen).
