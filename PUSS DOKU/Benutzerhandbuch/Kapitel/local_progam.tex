...
Auf Linux sollte das jeweilige Programm über die Kommandozeile mittels des Befehls \texttt{java -jar Planunungstool.jar} ausgeführt werden. 
...
Dafür exportieren wir die Daten aus UNTIS in sogenannte DIF-Dateien (Data Interchange Format) und übertragen diese anschließend in das Format der lokalen Planungsdatenbank.
...
Dabei hat der Planer die folgenden zwei Möglichkeiten, um die Daten von UNTIS in das lokale Planungstool zu übertragen:
\begin{itemize}
...
\end{itemize}
Wir empfehlen die erste Methode, da sie einfacher und schneller ist. Die zweite Methode bietet keine Mehrwert gegenüber der Methode, da Sie die Schritte des Importskripts manuell ausführt.
...
Diese Datei kann nun im lokalen Planungstool geöffnet werden und der importierte Stundenplan kann bearbeitet werden.
\subsubsection{Manueller Export der DIF-Dateien aus der UNTIS GUI}
...
Nachdem die DIF Dateien erstellt wurden, können diese in das lokale Planungstool importiert werden in dem man zunächst das Python Skript \textbf{dif-relevant-columns.py} ausführt.
Dies erstellt eine Datenbank mit dem Namen \textbf{UNTIS.db}, welche die Daten der DIF Dateien in eine Datenbank zusammenfasst.
Anschließend muss das Skript \textbf{dif-to-stulp.py} ausgeführt werden, um die Daten aus der UNTIS.db in das Format der lokalen Planungsdatenbank zu übertragen.
Nach der Ausführung dieses Skripts wird die Datei \textbf{STULP.db} erstellt, welche im lokalen Planungstool geöffnet werden kann.
\todo{Um Linux Anleitung ergänzen und vielleicht überarbeiten :).}
