...
Dies Unterschiede wollen wir im Folgenden kurz zusammenfassen und falls notwendig erläutern, wie die Begriffe der Schule in die Terminologie des lokalen Planungstools übersetzt werden können.
\begin{itemize}
	...
	\item \textbf{Nebenfach(NF)}: Dies habe wir nicht verwendet, da an der Schule keine Nebenfächer existieren. Es ist standardmäßig auf Ohne gesetzt.
	...
	\item \textbf{Veranstaltungsteil} Dies ist wohl die größte Abweichung in der Terminologie. Um eine Kompatibilität mit importierten UNTIS Stundenplänen zu gewährleisten, haben wir uns dazu entschieden den Begriff Veranstaltungsteil zweckzuentfremden.
	Ein Veranstaltungsteil ist beim Import eines UNTIS Stundenplans auch der zu dem Studiengang gehörigen Klasse zugeordnet.
\end{itemize}
\todo{Um weitere Begriffe ergänzen, wenn uns noch was einfällt.}
...
Falls bereits ein Stundenplan in Untis existiert, kann dieser importiert werden. Dafür exportieren wir die Daten aus UNTIS in sogenannte DIF-Dateien (Data Interchange Format). Dies sind strukturierte Textdateien, die anschließend vom lokalen Planungstool eingelesen werden.

Es gibt zwei Wege, diese Dateien zu erstellen:
\begin{enumerate}
    \item Direkter Export aus der UNTIS Benutzeroberfläche (GUI)
    \item Export via Command Line Interface (CLI) – für fortgeschrittene Planer
\end{enumerate}

Die erste Methode erfordert keine tiefgehenden technischen Kenntnisse. Die zweite Methode bietet hingegen den Vorteil, dass alle benötigten DIF-Dateien automatisiert in einem Rutsch exportiert werden können.
...
Standardmäßig ist in UNTIS oft ein anderer Zeichensatz eingestellt; dies muss manuell auf UTF-8 geändert werden. Bestätigen Sie die Einstellungen anschließend mit \textit{OK}.
...
\begin{itemize}
	...
	\item Zeitwünsche
\end{itemize}
\todo{Nochmal checken, ob das wirklich alle sind.}
...
\todo{Screenshots überarbeiten, wenn fertig.}
\begin{figure}[htbp]
    \centering
...
Die Veranstaltung dauert eine Stunde. \todo{Hier vielleicht anpassung des Beispiels nicht notwendig}
...
\begin{table}[ht]
	\begin{tabular}{|p{0.17\textwidth}|p{0.73\textwidth}|}
		...
		Professor& Veranstaltungen, die vom Professor gehalten werden\\
		Studiengang& Veranstaltungen, die Teil des Studiengangs sind\\
		Veranstaltung & Veranstaltungen, die zu der Veranstaltung gehören\\
...
\begin{enumerate}
	...
	\item ProfID des Professors, der die Veranstaltung hält
	\item Studiengänge, zu denen die Veranstaltung gehört. Falls der Veranstaltung zu mehr als einem Studiengang gehört, 
