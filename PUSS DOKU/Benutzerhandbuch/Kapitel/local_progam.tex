...
Dafür exportieren wir die Daten aus UNTIS in sogenannte DIF-Dateien (Data Interchange Format). 
Dies sind strukturierte Textdateien, die anschließend durch das Skript \todo{Name Einfügen} in das Format der lokalen Planungsdatenbank umgewandelt werden können.

Es gibt zwei Wege, diese Dateien zu erstellen:
\begin{enumerate}
    \item Direkter Export aus der UNTIS Benutzeroberfläche (GUI)
    \item Export via Command Line Interface (CLI)
\end{enumerate}
...
Für einen Konflikt werden jeweils die KursID in der Spalte KursID und die Klasse koodiert als Veranstaltungsteil in der Spalte Teil der beteiligten Veranstaltungsteile angezeigt.
