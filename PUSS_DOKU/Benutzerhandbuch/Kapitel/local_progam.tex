...
\begin{itemize}
	...
	Dabei ist der VeranstaltungsteilID eine kodierte Klasse zugeordnet. Dabei haben wir folgende Kodierung verwendet:
\end{itemize}
...
\subsubsection{Benötigte zusätzliche Software}
 Um das lokale Planungstool problemlos auszuführen, wird Java ($\geq 22$) benötigt.
 Falls außerdem ein bestehender Stundenplan von Untis in das lokale Planungstool übertragen werden soll, wird zusätzlich Python 3 und die Python-Bibliothek \texttt{pandas}\footnote{Die Installation von Pandas hängt vom Betriebssystem ab und kann leicht recherchiert werden.} benötigt, um die Dateien von Untis in das Format der lokalen Planungsdatenbank zu übertragen.
...
Auf Windows kann die jeweilige jar-Datei  mittels eines Doppelklicks ausgeführt werden.
Auf Linux sollte das jeweilige Programm über die Kommandozeile mittels folgendem Befehl ausgeführt werden:

\begin{verbatim}
    java -jar Planungstool.jar
\end{verbatim}
...
\end{itemize}
Wir empfehlen die erste Methode, da sie einfacher und schneller ist. Die zweite Methode bietet keinen Mehrwert gegenüber der ersten Methode, da sie die Schritte des Importskripts manuell ausführt.
...
Im Beispiel in \cref{fig:export_untis_step1} ist der Pfad zum Untis-Executable rot markiert und ohne Anführungszeichen angegeben. Sobald der Pfad eingegeben wurde, muss die Eingabe mit Enter bestätigt werden.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        ...
        \caption{Der Pfad zum Untis-Executable ist rot markiert. Enter drücken, um fortzufahren.}
...
\noindent % Verhindert Einrückung für das Layout
\begin{minipage}[t]{0.6\textwidth} % Linke Seite für den Text
    ...
    \begin{enumerate} 
        ...
        \item Auswählen der lokalen Planungsdatenbank
        \item Knopf \textbf{Öffnen} drücken
    \end{enumerate}
\end{minipage}
\hfill % Schiebt das Bild ganz nach rechts
\begin{minipage}[t]{0.35\textwidth} % Rechte Seite für das Bild
    \vspace{0pt} % Trick zur vertikalen Ausrichtung am oberen Rand
    \centering
    \includegraphics[width=\textwidth]{images/planning_tool/before_opening_db.png}
    \captionof{figure}{Datenbanken öffnen}
...
\paragraph{Konflikttabelle}\label{par:conf_table}
...
Für einen Konflikt werden jeweils die KursID in der Spalte KursID und die kodierte KursID als Veranstaltungsteil in der Spalte Teil der beteiligten Veranstaltungsteile angezeigt.
Zusätzlich wird in der Spalte Konflikt das Konfliktobjekt angezeigt. 
Dieses Konfliktobjekt ist entweder ein Lehrer, ein Raum oder eine Klasse.
...
Wenn im Plan nur die Konflikte mit einer Priorität größer als 3 (also Priorität 1 und 2) angezeigt werden sollen, kann der Schalter aktiviert werden und in dem Eingabefeld eine 2 eingetragen werden. 
...
\begin{figure}[htbp]
    ...
    \begin{minipage}[t]{0.45\textwidth}
        \centering
        \caption{Prioritäten zuweisen}
        \includegraphics[width=\linewidth]{images/planning_tool/conflict_prio_dialog.png}
        \label{fig:plt_conflict_prios}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.45\textwidth}
        ...
        \includegraphics[width=\linewidth]{images/planning_tool/pic08}
    ...
    \begin{minipage}[t]{0.45\textwidth}
        \centering
        \caption{Eigenen Konflikttyp erstellen}
        \includegraphics[width=\linewidth]{images/planning_tool/add_custom_conflict.png}
        \label{fig:plt_add_custom_conflict}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.45\textwidth}
        \centering
        \caption{Konflikte Menü}
        \includegraphics[width=\linewidth]{images/planning_tool/conflict_menu.png}
        \label{fig:plt_conflict_menu}
    \end{minipage}
...
%         \label{fig:plt_conflict_prios}
%     \end{minipage}
%     \hfill % Schiebt die Boxen an die Ränder (oder nutze \hspace{0.05\textwidth})
%     % Rechte Box (ehemals figure)
%     \begin{minipage}[t]{0.45\textwidth}
%         \centering
%         \caption{Eigene Konflikte verwalten}
%         \includegraphics[width=\linewidth]{images/planning_tool/pic08}
...
Im Menüabschnitt Konflikte können ignorierte Konflikte verwaltet werden. 
...
\paragraph{Unwiderrufliches Ignorieren von Konflikten mittels Konfigurationsdatei}
...
Pro Eintrag gilt, dass alle Konflikte zwischen den beiden Kursen ignoriert werden, sofern sie den gleichen Veranstaltungsteil haben, und der Veranstaltungsteil in der zugehörigen Veranstaltungsteilliste ist.
...
Dabei repräsentiert eine Zeile der Tabelle exakt eine ungeplante Veranstaltung. 
Für eine ungeplanten Veranstaltung wird die KursID in der Spalte KursID, die codierte Klasse in der Spalte Teil, der Lehrer in der Spalte ProfID und die Dauer der Veranstaltung in der Spalte Dauer angezeigt.\\ 
...
Dabei werden die, durch die Verschiebung entstandenen Änderungen an Raum und Zeit der Veranstaltung, gespeichert. 
...
\begin{figure}[htbp]
    % Linke Seite: Die Aufzählung
    \begin{minipage}[t]{0.55\textwidth}
        \vspace{0pt} % Erzwingt Ausrichtung an der Oberkante
        \begin{enumerate}
    ...
    \begin{minipage}[t]{0.4\textwidth}
        \vspace{0pt} % Erzwingt Ausrichtung an der Oberkante
        \centering
        \includegraphics[width=\linewidth]{images/planning_tool/menu_planung.png}
...
\begin{figure}[!ht] % Das ! ist entscheidend
    \centering
    
    % --- OBERES BILD ---
    % Breite auf 90% der Textbreite begrenzen, damit es nicht zu wuchtig wirkt
    \includegraphics[width=0.9\textwidth]{images/planning_tool/plan_window.png}
    \caption{Aufbau Stundenplanfenster}
    \label{fig:plt_pimary_plan}
    
    \vspace{0.5cm} % Kleinerer Abstand spart Platz
    
    % --- UNTERES BILD ---
    \includegraphics[width=0.9\textwidth]{images/planning_tool/plan_toolbar.png}
...
Eine Veranstaltung wird mithilfe eines Rechtecks an der von Tag und Uhrzeit bestimmten Stelle im Stundenplan angezeigt. 
Die folgenden Informationen werden im Rechteck für einen Veranstaltung in Abbildung \ref{fig:plt_single} angezeigt:

\begin{enumerate}
	\item KursID und Nummer die Veranstaltung
...
Wenn für Raum, Lehrer oder einer der Klassen ein Konflikt vorliegt wird dieser farbig hinterlegt. 
...
Die Veranstaltung \textit{D 9.4} findet im Raum \textit{B1.12} statt, wird von dem Lehrer mit der ProfID \textit{AM} gehalten und gehört zu der Klasse \textit{09D}.
...
Falls eine Veranstaltung nur partiell angezeigt wird, erscheint ein Warndreieck im zugehörigen Rechteck. 
...
\noindent % Verhindert Einrückung beim Start der Minipages
\begin{minipage}[t]{0.6\textwidth} % Linke Seite: 60% Breite für den Text
    \vspace{0pt} % Zwingt die Liste, oben zu starten (bündig mit Bild)
    \begin{enumerate}
        \item alle Konflikte 
    ...
    \end{enumerate}
\end{minipage}
\hfill % Schiebt den rechten Block nach rechts außen
\begin{minipage}[t]{0.35\textwidth} % Rechte Seite: 35% Breite für das Bild
    \vspace{0pt} % Zwingt das Bild, oben zu starten
    \centering
    % width=\linewidth passt das Bild automatisch an die 35% Breite an
    \includegraphics[width=\linewidth]{images/planning_tool/daten_check.png}
    \captionof{figure}{Daten überprüfen}
    \label{fig:plt_daten_check}
\end{minipage}

\medskip % Ein kleiner vertikaler Abstand vor dem Weiterlesen
...
\begin{figure}[h]
	...
	\includegraphics[scale=0.4]{images/planning_tool/settings.png}
...
\begin{figure}[!ht] % Das ! zwingt LaTeX, die Positionierung strenger durchzusetzen
    \centering
    
    % --- LINKE SEITE: TEXT ---
    \begin{minipage}[t]{0.55\textwidth} % Etwas breiter für den Text
        \vspace{0pt} % Wichtig für bündige Ausrichtung oben
        \begin{enumerate}
    ...
    \end{minipage}
    \hfill 
    % --- RECHTE SEITE: BILDER (UNTEREINANDER) ---
    \begin{minipage}[t]{0.4\textwidth} % Schmaler, damit Bilder kleiner wirken
        \vspace{0pt} % Wichtig für bündige Ausrichtung oben
        \centering
        ...
        \begin{subfigure}[b]{\linewidth} 
            \centering
            % width=0.7\linewidth macht das Bild innerhalb der Spalte kleiner (70%)
            \includegraphics[width=0.7\linewidth]{images/planning_tool/color_settings.png}
            \caption{}
            \label{fig:plt_menu_settings_farben}
        \end{subfigure}
        
        \vspace{0.5cm} % Abstand zwischen den Bildern
        
        % Zweites Unterbild (Jetzt darunter)
        \begin{subfigure}[b]{\linewidth}
            \centering
            \includegraphics[width=0.7\linewidth]{images/planning_tool/menu_settings_farben_choose.png}
            \caption{}
